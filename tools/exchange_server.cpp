...
//   oe_port      : OE TCP port
//   scenario.csv : optional path to CSV event file; omit for built-in scenario
//                  可选的 CSV 事件文件路径；省略则使用内置场景
//
// Example / 示例 (pair with hft binaries below):
//   exchange_server 127.0.0.1 12345 12346 0.0.0.0 9001
//   exchange_server 127.0.0.1 12345 12346 0.0.0.0 9001 scenarios/default.csv
...
int main(int argc, char **argv) {
  if (argc < 6) {
    fprintf(stderr,
            ...
            " <oe_addr> <oe_port> [scenario.csv]\n"
            "  md_dest      : UDP destination (unicast or multicast)\n"
            "  binary_port  : binary MD UDP port\n"
            "  fix_port     : FIX MD UDP port (0 = disabled)\n"
            "  oe_addr      : OE TCP listen address (e.g. 0.0.0.0)\n"
            "  oe_port      : OE TCP port\n"
            "  scenario.csv : CSV event file (omit for built-in scenario)\n");
    return 1;
  }
  ...
  if (argc > 6) cfg.scenario_csv = argv[6];
